...
data Expr' a
    ...
    | ExpEqs a (Expr' a) (EqsOp' a) (Expr' a)
...
type EqsOp = EqsOp' BNFC'Position
data EqsOp' a = OpEq a | OpNeq a
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)

type CmpOp = CmpOp' BNFC'Position
data CmpOp' a = OpGrt a | OpGeq a | OpLrt a | OpLeq a
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
instance HasPosition EqsOp where
  hasPosition = \case
    OpEq p -> p
    OpNeq p -> p
